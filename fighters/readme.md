...
## 游戏概述
战机大比拼是一款双人对战的回合制策略游戏，玩家通过选择不同等级的战机进行对决，最终根据总胜利次数决定游戏胜负。游戏采用了现代化的视觉设计和流畅的动画效果，支持在电脑、平板和手机上运行。

## 游戏功能
### 核心功能
1. 双人对战 ：支持两名玩家交替选择战机进行对决
2. 多种战机等级 ：游戏包含10种不同等级的战机，从轻型战斗机到外星战机，等级越高血量越高
3. 动态战场 ：战机在战场上随机分布，每次选择后位置会重新生成
4. 对决动画 ：战机对决时会播放导弹发射、爆炸等炫酷动画效果
5. 胜利音乐 ：游戏结束时播放胜利音乐，增强游戏体验
6. 战况记录 ：保存每轮对战记录，游戏结束后可查看详细战况
### 辅助功能
1. 游戏难度选择 ：支持10架、20架、30架、40架、50架战机的难度设置
2. 响应式设计 ：自动适配不同设备尺寸，支持手机横屏模式
3. 已选战机展示 ：底部显示双方已选战机列表，方便玩家查看
4. 轮次计数 ：实时显示当前轮次和双方得分
5. 横屏提示 ：在手机竖屏时提示用户切换到横屏模式
## 游戏规则
### 基本规则
1. 游戏流程 ：
   ...
   - 玩家1和玩家2交替从战场中选择战机
   - 每轮双方各选择1架战机后，进行对决
   - 对决结果根据战机血量决定，血量高的一方获胜
   - 所有战机选择完毕后，游戏结束，统计总胜利次数
2. 战机等级与血量 ：
...
4. 先手规则 ：
   ...
   - 第一轮随机决定先手玩家
   - 后续每轮由上一轮的胜者先手
   - 若上一轮平局，则由当前轮次的先手玩家的另一方先手
## 操作说明
### 电脑操作
1. 点击游戏难度按钮选择战机数量
2. 根据游戏提示，玩家交替点击战场中的战机卡片进行选择
3. 战机对决结束后，点击「继续游戏」按钮进入下一轮
4. 游戏结束后，可选择「查看战况」查看详细记录，或选择「再来一局」重新开始
### 手机/平板操作
1. 游戏会自动提示横屏，将设备横屏后开始游戏
2. 点击游戏难度按钮选择战机数量
3. 玩家交替点击战场中的战机卡片进行选择
4. 点击底部「已选战机」横幅可展开/收起已选战机列表
5. 游戏结束后，可选择「查看战况」查看详细记录，或选择「再来一局」重新开始
## 技术说明
### 音频播放优化
- 采用多层次音频播放策略，确保在各种设备上正常播放胜利音乐
- 支持HTML5 Audio元素和Web Audio API
- 针对移动设备进行了专门优化，解决了平板和手机上的音频播放问题
### 视觉设计
- 采用现代化的UI设计，包含渐变色彩、阴影效果和流畅动画
- 战场背景随机切换，增强游戏视觉多样性
- 战机对决时的动画效果包括导弹飞行、爆炸和震动效果
### 响应式设计
- 自动适配不同屏幕尺寸
- 手机端采用折叠式已选战机列表，优化屏幕空间利用
- 横屏提示确保最佳游戏体验
## 游戏提示词
### 游戏开始提示
- 「66小朋友，请选择你的战机」
...
- V1.2 ：增强了对决动画效果
## 开发者信息
本游戏采用纯HTML、CSS和JavaScript开发，无任何外部依赖，可直接在浏览器中运行。

游戏名称 ：战机大比拼 (Fighters) 游戏类型 ：双人对战策略游戏 支持平台 ：PC、平板、手机 游戏语言 ：中文 开发技术 ：HTML5、CSS3、JavaScript
